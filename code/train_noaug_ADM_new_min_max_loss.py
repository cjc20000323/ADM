...
def main():
    ...
    statistics_dict = {'min_max_loss': min_max_loss_list, 'min_loss': min_loss_list, 'max_loss': max_loss_list}
    file = open(f'file/min_max_loss_ag_{args.n_labeled}.txt', 'w')
...
def train(labeled_trainloader, unlabeled_trainloader, unlabeled_labeled_trainloader, model, outlier_detection_model,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        min_max_loss_list.append(loss.item())
        min_loss_list.append(loss.item())

        writer.add_scalar('loss by step', loss.item(), steps)
        if steps < 1000:
            writer.add_scalar('loss by step less 1000', loss.item(), steps)
        if steps < 2000:
            writer.add_scalar('loss by step less 2000', loss.item(), steps)
        if steps < 4000:
            writer.add_scalar('loss by step less 4000', loss.item(), steps)
        if steps < 6000:
            writer.add_scalar('loss by step less 6000', loss.item(), steps)
        if steps < 8000:
            writer.add_scalar('loss by step less 8000', loss.item(), steps)
        if steps < 10000:
            writer.add_scalar('loss by step less 10000', loss.item(), steps)
        ...
        min_max_loss_list.append(loss.item())
        max_loss_list.append(loss.item())
