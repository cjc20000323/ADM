...
def main():
    ...
    all_test_f1s = []
    all_test_f1s1 = []
    actual_normal_sum_list = []
    actual_abnormal_sum_list = []
    judge_normal_sum_list = []
    judge_abnormal_sum_list = []
    both_normal_sum_list = []
    both_abnormal_sum_list = []
    both_to_actual_normal_list = []
    both_to_judge_normal_list = []
    both_to_actual_abnormal_list = []
    both_to_judge_abnormal_list = []
    both_to_actual_list = []
    both_to_judge_list = []
    val_acc_list = []
    val_f1_list = []
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        actual_normal_sum, actual_abnormal_sum, judge_normal_sum, judge_abnormal_sum, both_normal_sum, \
        both_abnormal_sum, both_to_actual_normal, both_to_judge_normal, both_to_actual_abnormal, \
        both_to_judge_abnormal, both_to_actual, both_to_judge = val_statistics(val_tune_loader, model,
                                                                               outlier_detection_model, epoch)

        actual_normal_sum_list.append(actual_normal_sum)
        actual_abnormal_sum_list.append(actual_abnormal_sum)
        judge_normal_sum_list.append(judge_normal_sum)
        judge_abnormal_sum_list.append(judge_abnormal_sum)
        both_normal_sum_list.append(both_normal_sum)
        both_abnormal_sum_list.append(both_abnormal_sum)
        both_to_actual_normal_list.append(both_to_actual_normal)
        both_to_judge_normal_list.append(both_to_judge_normal)
        both_to_actual_abnormal_list.append(both_to_actual_abnormal)
        both_to_judge_abnormal_list.append(both_to_judge_abnormal)
        both_to_actual_list.append(both_to_actual)
        both_to_judge_list.append(both_to_judge)
        val_acc_list.append(val_acc1)
        val_f1_list.append(val_f11)

        writer.add_scalar('actual_normal_sum', actual_normal_sum, epoch)
        writer.add_scalar('actual_abnormal_sum', actual_abnormal_sum, epoch)
        writer.add_scalar('judge_normal_sum', judge_normal_sum, epoch)
        writer.add_scalar('judge_abnormal_sum', judge_abnormal_sum, epoch)
        writer.add_scalar('both_normal_sum', both_normal_sum, epoch)
        writer.add_scalar('both_abnormal_sum', both_abnormal_sum, epoch)
        writer.add_scalar('both_to_actual_normal', both_to_actual_normal, epoch)
        writer.add_scalar('both_to_judge_normal', both_to_judge_normal, epoch)
        writer.add_scalar('both_to_actual_abnormal', both_to_actual_abnormal, epoch)
        writer.add_scalar('both_to_judge_abnormal', both_to_judge_abnormal, epoch)
        writer.add_scalar('both_to_actual', both_to_actual, epoch)
        writer.add_scalar('both_to_judge', both_to_judge, epoch)
    ...
    statistics_dict = {'actual_normal_sum': actual_normal_sum_list, 'actual_abnormal_sum': actual_abnormal_sum_list,
                       'judge_normal_sum': judge_normal_sum_list, 'judge_abnormal_sum': judge_abnormal_sum_list,
                       'both_normal_sum': both_normal_sum_list, 'both_abnormal_sum': both_abnormal_sum_list,
                       'both_to_actual_normal': both_to_actual_normal_list,
                       'both_to_judge_normal_list': both_to_judge_normal_list,
                       'both_to_actual_abnormal': both_to_actual_abnormal_list,
                       'both_to_judge_abnormal': both_to_judge_abnormal_list,
                       'both_to_actual': both_to_actual_list, 'both_to_judge': both_to_judge_list,
                       'val_acc': val_acc_list, 'val_f1': val_f1_list}
    file = open(f'./file/statistics_ag_{args.n_labeled}.txt', 'w')
    data_str = str(statistics_dict)
    file.write(data_str)
    file.close()
...
def val_statistics(valloader, model, outlier_detection_model, epoch):
    model.eval()
    outlier_detection_model.eval()
    ...
    with torch.no_grad():
        actual_normal_information = []  # 该列表表明在验证集当中，各个数据实际正常和异常情况，1代表正常，0代表异常
        judge_normal_information = []  # 该列表表明在验证集当中各个数据判断的正常和异常情况，1代表正常，0代表异常

        for batch_idx, (inputs, targets, targets_tune, length) in enumerate(valloader):
            inputs, targets = inputs.cuda(), targets.cuda(non_blocking=True)  # targets [batch_size]
            targets_tune = targets_tune.cuda()
            outputs, embedding = model(inputs)
            _, predicted = torch.max(outputs.data, 1)
            predicted1 = torch.clone(predicted)
            ...
            # 在这个列表中，存储当前列表中实际的正常异常情况，实际的标签和预测结果不符，也就是实际的异常，那么记为0，相符，也就是实际的正常，记为1

            actual_normal_information.extend(actual_normal_information_list.tolist())
            ...
            lamda[torch.where(beta <= 0.5)] = 1
            alpha[torch.where(abs_value > args.theta_threshold)] = 1
            alpha[torch.where(abs_value <= args.theta_threshold)] = 0  # [batch_size, 1]
            # alpha就是判断是否是异常的，在训练时，alpha是0那么就是正常，alpha为1那么就是异常，但是验证集不参与训练，所以为了统一让正常是1，异常是0

            judge_normal_information_list = alpha.squeeze().tolist()
            # 这个是该批次数据判断的正常与异常情况
            judge_normal_information.extend(judge_normal_information_list)

        actual_normal_sum = sum(actual_normal_information)  # 实际正常总数
        actual_abnormal_sum = len(actual_normal_information) - actual_normal_sum  # 实际异常总数
        judge_normal_sum = sum(judge_normal_information)  # 判断正常总数
        judge_abnormal_sum = len(judge_normal_information) - judge_normal_sum  # 判断异常总数

        actual_normal_information = np.array(actual_normal_information)
        judge_normal_information = np.array(judge_normal_information)

        both_normal_sum = np.sum(actual_normal_information[np.where(judge_normal_information == 1)])  # 这是实际正常判断也是正常的总数
        both_abnormal_sum = judge_abnormal_sum - np.sum(
            actual_normal_information[np.where(judge_normal_information == 0)])  # 这是实际异常判断也是异常的总数
        both_sum = both_abnormal_sum + both_normal_sum  # 这是实际和判断相同的总数

        both_to_actual_normal = both_normal_sum / actual_normal_sum
        both_to_judge_normal = both_normal_sum / judge_normal_sum
        both_to_actual_abnormal = both_abnormal_sum / actual_abnormal_sum
        both_to_judge_abnormal = both_abnormal_sum / judge_abnormal_sum
        both_to_actual = both_sum / (actual_abnormal_sum + actual_normal_sum)
        both_to_judge = both_sum / (judge_abnormal_sum + judge_normal_sum)

        return actual_normal_sum, actual_abnormal_sum, judge_normal_sum, judge_abnormal_sum, both_normal_sum, \
               both_abnormal_sum, both_to_actual_normal, both_to_judge_normal, both_to_actual_abnormal, \
               both_to_judge_abnormal, both_to_actual, both_to_judge
