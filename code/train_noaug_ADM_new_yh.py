...
parser.add_argument('--outlier-threshold', default=0.5, type=float)
...
print('Outlier threshold', args.outlier_threshold)
...
def train(labeled_trainloader, unlabeled_trainloader, unlabeled_labeled_trainloader, model, outlier_detection_model,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        lamda[torch.where(beta > outlier_threshold)] = 0
        lamda[torch.where(beta <= outlier_threshold)] = 1
...
def test_out_of_distribution_set(testloader, model, outlier_detection_model):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, targets_tune, length) in enumerate(testloader):
            ...
            wrong_out_detection += np.sum(predicted_out_detection_array[np.where(targets_tune_array == -1)] > outlier_threshold)
            wrong_out_detection1 += np.sum(predicted_out_detection_array[np.where(targets_tune_array == 1)] <= outlier_threshold)

            predicted_out_detection_array[np.where(predicted_out_detection_array > outlier_threshold)] = 1
            predicted_out_detection_array[np.where(predicted_out_detection_array <= outlier_threshold)] = -1
...
def test_all_set(testloader, model, outlier_detection_model, epoch, mode='val'):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, targets_tune, length) in enumerate(testloader):
            ...
            predicted1[torch.where(predicted_out_detection.le(outlier_threshold))] = -1
