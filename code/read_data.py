...
def get_data_tune(data_path, n_labeled_per_class, unlabeled_per_class=5000, max_seq_len=256, model='bert-base-uncased',
    ...
    train_labels_tune = np.array(labels_list)  # 调整后的训练集标签，1代表在ID里面，0代表OOD
    ...
    for u in test_df[0]:
        if u - 1 in in_distribution_train_labels_set:
            ...
            labels_list.append(0)
    test_labels_tune = np.array(labels_list)  # 调整后的测试集标签，1代表ID，0代表OOD
    ...
    out_distribution_test_labels_tune = np.array(labels_list)  # 调整后的OOD标签，应该全部是0
    out_distribution_test_text = np.array(out_distribution_test_text)
    out_distribution_test_labels = np.array(out_distribution_test_labels)  # 假设给OOD分配的调整前标签是-1
    ...
    test_idxs = test_split(test_labels, n_labeled_per_class, unlabeled_per_class, n_labels, seed=seed,
                           in_dis_labels_set=in_distribution_train_labels_set, labels_set=train_labels_set)
    print(train_text[val_idxs[:50]])
    print(train_labels[val_idxs[:50]])
    '''
    val_dict = {'train_text': train_text[val_idxs[:50]], 'train_labels': train_labels[val_idxs[:50]]}
    file = open(f'./file/val_ag_{n_labeled_per_class}.txt', 'w')
    data_str = str(val_dict)
    file.write(data_str)
    file.close()
    '''
    ...
    for i, u in enumerate(train_df[0]):
        if u - 1 in in_distribution_train_labels_set:
            ...
            train_labels[i] = -1
    # 这里，把所有的ID数据标签调成ID label list中对应的索引，OOD数据全调成-1
...
def train_val_split(labels, n_labeled_per_class, unlabeled_per_class, n_labels, seed=0, in_dis_labels_set=None,
    ...
    train_labeled_idxs = []  # 训练数据中有标签编号
    train_unlabeled_idxs = []  # 训练数据中无标签编号
    val_idxs = []  # 验证集标签编号
    in_distribution_train_labeled_idxs = []  # 保存in_distribution部分中有标签训练数据
