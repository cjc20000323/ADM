...
def main():
    ...
    if args.use_pretrain:
        pre_train(labeled_trainloader, unlabeled_trainloader, model, outlier_detection_model, model, optimizer,
                  ...
                  in_distribution_test_loader, test_tune_loader)
    ...
    all_judge_abnormal_result_tensor = torch.tensor(all_judge_abnormal_result)
    all_true_abnormal_result_tensor = torch.tensor(all_true_abnormal_result)

    manual_all_right_sum = torch.sum(all_true_abnormal_result_tensor == all_judge_abnormal_result_tensor)

    manual_all_right_score = manual_all_right_sum / all_sample_sum

    machine_all_right_score = accuracy_score(all_judge_abnormal_result, all_true_abnormal_result)

    manual_all_precision_sum = torch.sum(
        all_judge_abnormal_result_tensor[torch.where(all_true_abnormal_result_tensor == 1)] == 1)

    manual_all_precision_score = manual_all_precision_sum / torch.sum(all_true_abnormal_result_tensor)

    machine_all_precision_score = precision_score(all_judge_abnormal_result, all_true_abnormal_result)

    print('Manual all right sum:', manual_all_right_sum.item())
    print('Manual all right score:', manual_all_right_score.item() / 2)
    print('Machine all right score:', machine_all_right_score)
    print('Manual all precision sum:', manual_all_precision_sum.item())
    print('Manual all precision score:', manual_all_precision_score.item())
    print('Machine all precision score:', machine_all_precision_score)
...
def pre_train(labeled_trainloader, unlabeled_trainloader, model, outlier_detection_model, EM_model, optimizer,
              optimizer_beta,
              scheduler, criterion, sigmoid_criterion, epoch, testloader, in_distribution_loader, loader):
    global best_pre_acc
    ...
    for batch_id in tqdm(range(args.pretrain_inititeration)):
        ...
        if batch_id % args.check_inititeration == 0:
            val_acc, val_acc1, val_f1, val_f11 = test_all_set(testloader, model, outlier_detection_model, epoch,
                                                              mode='test')
            in_distribution_test_loss, in_distribution_test_acc = validate(
                in_distribution_loader, model, outlier_detection_model, criterion, epoch, mode='Pre')
            out_test_err, out_test_err1, out_test_outlier_detection_err, out_test_outlier_detection_err1, out_test_acc = test_out_of_distribution_set(
                loader, model, outlier_detection_model)
            all_acc, all_acc1, all_f1, all_f11 = test_all_set(
                loader, model, outlier_detection_model, epoch, mode='test')
            ...
            writer.add_scalar('in_distribution_test_acc_pretrain_init', in_distribution_test_acc, batch_id)
            writer.add_scalar('out_test_acc_pretrain_init', out_test_acc, batch_id)
            writer.add_scalar('acc_pretrain_init', all_acc1, batch_id)
    ...
    for batch_id in tqdm(range(args.pretrain_afteriteration)):
        ...
        if batch_id % args.check_afteriteration == 0:
            ...
            writer.add_scalar('in_distribution_test_acc_pretrain_after', in_distribution_test_acc, batch_id)
            writer.add_scalar('out_test_acc_pretrain_after', out_test_acc, batch_id)
            writer.add_scalar('acc_pretrain_after', all_acc1, batch_id)
...
def train(labeled_trainloader, unlabeled_trainloader, unlabeled_labeled_trainloader, model, outlier_detection_model,
    ...
    epoch_sample_sum = 0
    epoch_judge_abnormal_sum_1 = 0
    epoch_judge_abnormal_sum_2 = 0
    epoch_judge_abnormal_sum = 0
    epoch_true_abnormal_sum_1 = 0
    epoch_true_abnormal_sum_2 = 0
    epoch_true_abnormal_sum = 0
    epoch_judge_normal_sum = 0
    epoch_judge_normal_sum_1 = 0
    epoch_judge_normal_sum_2 = 0
    epoch_true_normal_sum = 0
    epoch_true_normal_sum_1 = 0
    epoch_true_normal_sum_2 = 0

    epoch_true_abnormal_result = []
    epoch_true_abnormal_result_1 = []
    epoch_true_abnormal_result_2 = []
    epoch_judge_abnormal_result = []
    epoch_judge_abnormal_result_1 = []
    epoch_judge_abnormal_result_2 = []
    epoch_true_normal_result = []
    epoch_true_normal_result_1 = []
    epoch_true_normal_result_2 = []
    epoch_judge_normal_result = []
    epoch_judge_normal_result_1 = []
    epoch_judge_normal_result_2 = []
    ...
    global writer
    global steps
    global all_sample_sum
    global all_judge_abnormal_sum
    global all_true_abnormal_sum
    global all_true_normal_sum
    ...
    for batch_idx in range(args.val_iteration):
        ...
        if train_aug:
            ...
            epoch_sample_sum += batch_size_2
            all_sample_sum += batch_size_2
        ...
        epoch_judge_abnormal_sum += torch.sum(alpha)
        epoch_judge_abnormal_sum_1 += torch.sum(alpha)
        all_judge_abnormal_sum += torch.sum(alpha)

        epoch_judge_abnormal_result.extend(np.array(alpha.squeeze().cpu().detach().numpy()))
        epoch_judge_abnormal_result_1.extend(np.array(alpha.squeeze().cpu().detach().numpy()))
        all_judge_abnormal_result.extend(np.array(alpha.squeeze().cpu().detach().numpy()))
        epoch_judge_normal_result.extend(np.array((1 - alpha).squeeze().cpu().detach().numpy()))
        epoch_judge_normal_result_1.extend(np.array((1 - alpha).squeeze().cpu().detach().numpy()))
        all_judge_normal_result.extend(np.array((1 - alpha).squeeze().cpu().detach().numpy()))

        targets_u_labeled_clone = targets_u_labeled.unsqueeze(1)  # [8,1] beta [8,1]
        targets_u_labeled_clone[torch.where(targets_u_labeled_clone != -1)] = 0
        targets_u_labeled_clone[torch.where(targets_u_labeled_clone == -1)] = 1
        # 这里调整为，当真实标签为out时，调成1，否则为0，与lamda代表同样含义

        true_abnormal_result = torch.ne(targets_u_labeled_clone, lamda).long()
        true_normal_result = torch.eq(targets_u_labeled_clone, lamda).long()
        # 这个代表实际的异常样本序列，True代表是，False代表不是 [8,1]
        epoch_true_abnormal_result.extend(np.array(true_abnormal_result.squeeze().cpu().detach().numpy()))
        epoch_true_abnormal_result_1.extend(np.array(true_abnormal_result.squeeze().cpu().detach().numpy()))
        epoch_true_normal_result.extend(np.array(true_normal_result.squeeze().cpu().detach().numpy()))
        epoch_true_normal_result_1.extend(np.array(true_normal_result.squeeze().cpu().detach().numpy()))
        all_true_abnormal_result.extend(np.array(true_abnormal_result.squeeze().cpu().detach().numpy()))
        all_true_normal_result.extend(np.array(true_abnormal_result.squeeze().cpu().detach().numpy()))

        epoch_true_abnormal_sum += torch.sum(torch.ne(targets_u_labeled_clone, lamda)).item()
        epoch_true_abnormal_sum_1 += torch.sum(torch.ne(targets_u_labeled_clone, lamda)).item()
        all_true_abnormal_sum += torch.sum(torch.ne(targets_u_labeled_clone, lamda)).item()

        epoch_true_normal_sum += torch.sum(torch.eq(targets_u_labeled_clone, lamda)).item()
        epoch_true_normal_sum_1 += torch.sum(torch.eq(targets_u_labeled_clone, lamda)).item()
        all_true_normal_sum += torch.sum(torch.eq(targets_u_labeled_clone, lamda)).item()
        ...
        epoch_judge_abnormal_sum += torch.sum(alpha)
        epoch_judge_abnormal_sum_2 += torch.sum(alpha)
        all_judge_abnormal_sum += torch.sum(alpha)

        epoch_judge_abnormal_result.extend(np.array(alpha.squeeze().cpu().detach().numpy()))
        epoch_judge_abnormal_result_2.extend(np.array(alpha.squeeze().cpu().detach().numpy()))
        all_judge_abnormal_result.extend(np.array(alpha.squeeze().cpu().detach().numpy()))
        epoch_judge_normal_result.extend(np.array((1 - alpha).squeeze().cpu().detach().numpy()))
        epoch_judge_normal_result_2.extend(np.array((1 - alpha).squeeze().cpu().detach().numpy()))
        all_judge_normal_result.extend(np.array((1 - alpha).squeeze().cpu().detach().numpy()))
        ...
        epoch_true_abnormal_sum += torch.sum(torch.ne(targets_u_labeled_clone, lamda)).item()
        epoch_true_abnormal_sum_2 += torch.sum(torch.ne(targets_u_labeled_clone, lamda)).item()
        all_true_abnormal_sum += torch.sum(torch.ne(targets_u_labeled_clone, lamda)).item()
        ...
        true_normal_result = torch.eq(targets_u_labeled_clone, lamda).long()
        epoch_true_abnormal_result.extend(np.array(true_abnormal_result.squeeze().cpu().detach().numpy()))
        epoch_true_abnormal_result_2.extend(np.array(true_abnormal_result.squeeze().cpu().detach().numpy()))
        epoch_true_normal_result.extend(np.array(true_normal_result.squeeze().cpu().detach().numpy()))
        epoch_true_normal_result_2.extend(np.array(true_normal_result.squeeze().cpu().detach().numpy()))
        all_true_abnormal_result.extend(np.array(true_abnormal_result.squeeze().cpu().detach().numpy()))
        all_true_normal_result.extend(np.array(true_abnormal_result.squeeze().cpu().detach().numpy()))
    ...
    writer.add_scalar('epoch_judge_abnormal_sum_1', epoch_judge_abnormal_sum_1, epoch)
    writer.add_scalar('epoch_judge_abnormal_sum_2', epoch_judge_abnormal_sum_2, epoch)
    writer.add_scalar('epoch_judge_abnormal_sum', epoch_judge_abnormal_sum, epoch)
    writer.add_scalar('epoch_true_abnormal_sum_1', epoch_true_abnormal_sum_1, epoch)
    writer.add_scalar('epoch_true_abnormal_sum_2', epoch_true_abnormal_sum_2, epoch)
    writer.add_scalar('epoch_true_abnormal_sum', epoch_true_abnormal_sum, epoch)

    epoch_judge_abnormal_result_tensor = torch.tensor(epoch_judge_abnormal_result)
    epoch_judge_abnormal_result_tensor_1 = torch.tensor(epoch_judge_abnormal_result_1)
    epoch_judge_abnormal_result_tensor_2 = torch.tensor(epoch_judge_abnormal_result_2)

    epoch_true_abnormal_result_tensor = torch.tensor(epoch_true_abnormal_result)
    epoch_true_abnormal_result_tensor_1 = torch.tensor(epoch_true_abnormal_result_1)
    epoch_true_abnormal_result_tensor_2 = torch.tensor(epoch_true_abnormal_result_2)

    epoch_true_normal_result_tensor = torch.tensor(epoch_true_normal_result)
    epoch_true_normal_result_tensor_1 = torch.tensor(epoch_true_normal_result_1)
    epoch_true_normal_result_tensor_2 = torch.tensor(epoch_true_normal_result_2)

    manual_epoch_right_sum = torch.sum(epoch_judge_abnormal_result_tensor == epoch_true_abnormal_result_tensor)
    manual_epoch_right_sum_1 = torch.sum(epoch_judge_abnormal_result_tensor_1 == epoch_true_abnormal_result_tensor_1)
    manual_epoch_right_sum_2 = torch.sum(epoch_judge_abnormal_result_tensor_2 == epoch_true_abnormal_result_tensor_2)

    writer.add_scalar('manual_epoch_right_sum', manual_epoch_right_sum, epoch)
    writer.add_scalar('manual_epoch_right_sum_1', manual_epoch_right_sum_1, epoch)
    writer.add_scalar('manual_epoch_right_sum_2', manual_epoch_right_sum_2, epoch)

    manual_epoch_right_score = manual_epoch_right_sum / epoch_sample_sum
    manual_epoch_right_score_1 = manual_epoch_right_sum_1 / epoch_sample_sum
    manual_epoch_right_score_2 = manual_epoch_right_sum_2 / epoch_sample_sum

    writer.add_scalar('manual_epoch_right_score', manual_epoch_right_score / 2, epoch)
    writer.add_scalar('manual_epoch_right_score_1', manual_epoch_right_score_1, epoch)
    writer.add_scalar('manual_epoch_right_score_2', manual_epoch_right_score_2, epoch)

    machine_epoch_right_score = accuracy_score(epoch_judge_abnormal_result, epoch_true_abnormal_result)
    machine_epoch_right_score_1 = accuracy_score(epoch_judge_abnormal_result_1, epoch_true_abnormal_result_1)
    machine_epoch_right_score_2 = accuracy_score(epoch_judge_abnormal_result_2, epoch_true_abnormal_result_2)

    writer.add_scalar('machine_epoch_right_score', machine_epoch_right_score, epoch)
    writer.add_scalar('machine_epoch_right_score_1', machine_epoch_right_score_1, epoch)
    writer.add_scalar('machine_epoch_right_score_2', machine_epoch_right_score_2, epoch)

    manual_epoch_precision_sum = torch.sum(
        epoch_judge_abnormal_result_tensor[torch.where(epoch_true_abnormal_result_tensor == 1)] == 1)
    manual_epoch_precision_sum_1 = torch.sum(
        epoch_judge_abnormal_result_tensor_1[torch.where(epoch_true_abnormal_result_tensor_1 == 1)] == 1)
    manual_epoch_precision_sum_2 = torch.sum(
        epoch_judge_abnormal_result_tensor_2[torch.where(epoch_true_abnormal_result_tensor_2 == 1)] == 1)

    writer.add_scalar('manual_epoch_precision_sum', manual_epoch_precision_sum, epoch)
    writer.add_scalar('manual_epoch_precision_sum_1', manual_epoch_precision_sum_1, epoch)
    writer.add_scalar('manual_epoch_precision_sum_2', manual_epoch_precision_sum_2, epoch)

    manual_epoch_normal_precision_sum = torch.sum(
        epoch_judge_abnormal_result_tensor[torch.where(epoch_true_abnormal_result_tensor == 0)] == 0)
    manual_epoch_normal_precision_sum_1 = torch.sum(
        epoch_judge_abnormal_result_tensor_1[torch.where(epoch_true_abnormal_result_tensor_1 == 0)] == 0)
    manual_epoch_normal_precision_sum_2 = torch.sum(
        epoch_judge_abnormal_result_tensor_2[torch.where(epoch_true_abnormal_result_tensor_2 == 0)] == 0)

    writer.add_scalar('manual_epoch_normal_precision_sum', manual_epoch_normal_precision_sum, epoch)
    writer.add_scalar('manual_epoch_normal_precision_sum_1', manual_epoch_normal_precision_sum_1, epoch)
    writer.add_scalar('manual_epoch_normal_precision_sum_2', manual_epoch_normal_precision_sum_2, epoch)

    manual_epoch_precision_score = manual_epoch_precision_sum / torch.sum(epoch_true_abnormal_result_tensor)
    manual_epoch_precision_score_1 = manual_epoch_precision_sum_1 / torch.sum(epoch_true_abnormal_result_tensor_1)
    manual_epoch_precision_score_2 = manual_epoch_precision_sum_2 / torch.sum(epoch_true_abnormal_result_tensor_2)

    writer.add_scalar('manual_epoch_precision_score', manual_epoch_precision_score, epoch)
    writer.add_scalar('manual_epoch_precision_score_1', manual_epoch_precision_score_1, epoch)
    writer.add_scalar('manual_epoch_precision_score_2', manual_epoch_precision_score_2, epoch)

    manual_epoch_normal_precision_score = manual_epoch_normal_precision_sum / torch.sum(epoch_true_normal_result_tensor)
    manual_epoch_normal_precision_score_1 = manual_epoch_normal_precision_sum_1 / torch.sum(
        epoch_true_normal_result_tensor_1)
    manual_epoch_normal_precision_score_2 = manual_epoch_normal_precision_sum_2 / torch.sum(
        epoch_true_normal_result_tensor_2)

    writer.add_scalar('manual_epoch_normal_precision_score', manual_epoch_normal_precision_score, epoch)
    writer.add_scalar('manual_epoch_normal_precision_score_1', manual_epoch_normal_precision_score_1, epoch)
    writer.add_scalar('manual_epoch_normal_precision_score_2', manual_epoch_normal_precision_score_2, epoch)

    machine_epoch_precision_score = precision_score(epoch_judge_abnormal_result, epoch_true_abnormal_result)
    machine_epoch_precision_score_1 = precision_score(epoch_judge_abnormal_result_1, epoch_true_abnormal_result_1)
    machine_epoch_precision_score_2 = precision_score(epoch_judge_abnormal_result_2, epoch_true_abnormal_result_2)

    writer.add_scalar('machine_epoch_precision_score', machine_epoch_precision_score, epoch)
    writer.add_scalar('machine_epoch_precision_score_1', machine_epoch_precision_score_1, epoch)
    writer.add_scalar('machine_epoch_precision_score_2', machine_epoch_precision_score_2, epoch)

    machine_epoch_normal_precision_score = precision_score(epoch_judge_normal_result, epoch_true_normal_result)
    machine_epoch_normal_precision_score_1 = precision_score(epoch_judge_normal_result_1, epoch_true_normal_result_1)
    machine_epoch_normal_precision_score_2 = precision_score(epoch_judge_normal_result_2, epoch_true_normal_result_2)

    writer.add_scalar('machine_epoch_normal_precision_score', machine_epoch_normal_precision_score, epoch)
    writer.add_scalar('machine_epoch_normal_precision_score_1', machine_epoch_normal_precision_score_1, epoch)
    writer.add_scalar('machine_epoch_normal_precision_score_2', machine_epoch_normal_precision_score_2, epoch)
