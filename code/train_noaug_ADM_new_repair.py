...
def train(labeled_trainloader, unlabeled_trainloader, unlabeled_labeled_trainloader, model, outlier_detection_model,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        loss_list_u = new_criterion(outputs_u, predicted.to(torch.int64))
        loss_list_u2 = new_criterion(outputs_u2, predicted_u2.to(torch.int64))
        loss_list_ori = new_criterion(outputs_ori, predicted_ori.to(torch.int64))
        loss_list = torch.cat((loss_list_u, loss_list_u2, loss_list_ori), dim=0).unsqueeze(1)
        embedding = torch.cat((embedding_u, embedding_u2, embedding_ori), dim=0)

        beta = outlier_detection_model(embedding)  # [20, 1]
        ...
        targets_u_labeled_clone = torch.cat((targets_u_labeled, targets_u_labeled, targets_u_labeled)).unsqueeze(1)  # [8,1] beta [8,1]
